...
class PolarMapper(object):
    ...
    def save_bokeh_points(self,output_file_name,binder=None, n_maxima=10):
        ...
        points, theta_angle, enrichment_r = self.plot_points(binder, extra_tools=[],n_maxima=n_maxima)
        ellipse_diameter = enrichment_r.max() * 2
        
        fist_missing_local_maxima = self.local_maxima[binder][0][n_maxima]
        first_excluded_diameter = 2*self.data.loc[binder].loc[fist_missing_local_maxima, self.enrichment_column]
        exc_ell = hv.Ellipse(0, 0, first_excluded_diameter)
        hv.opts({"Ellipse":{"style":{"color":"red","alpha":0.5}}},exc_ell)

        points = exc_ell * \
            points* self.kmer_annotation_overlay(theta_angle, enrichment_r) * \
            hv.Ellipse(0, 0, ellipse_diameter) 
            
        # for fist_missing_local_maxima in self.local_maxima[binder][0][:(n_maxima+1)]:
        #     first_excluded_diameter = 2*self.data.loc[binder].loc[fist_missing_local_maxima, self.enrichment_column]
        #     print(fist_missing_local_maxima,first_excluded_diameter)
        #     points = points*hv.Ellipse(0, 0, first_excluded_diameter) 
        ...
        The plot displays {n_maxima:d} locally maximal kmers and their neighbourhoods. 
        ...
        The {n_maxima:d} local maxima are set to locations minimizing the sum of squared error 
        ...
        non-maximal points. The outer circle is at the globally maximal enrichment and the inner one
        is at the first excluded local maxima.<p>
        {tag}
        This file is generated by {pkg.key} {pkg.version} at {time}.
        Above plot can be embeded in a HTML document by including 
        <quote>{escaped_tag}</quote>.
        </html>"""
        ...
        with open(js_name,"w") as js_f, open(html_name,"w") as html_f:
            ...
            html_f.write(t.format(binder=binder,
                ...
                tag=tag,
                n_maxima=n_maxima,
                escaped_tag = cgi.escape(tag, quote=True),
    ...
    def plot_points(self, binder, extra_tools =  ['box_select', 'lasso_select',"wheel_zoom"],n_maxima=10):
        import pandas as pd
        import logging as log
        import numpy as np
        import holoviews as hv
    ...
    def kmer_annotation_overlay(self, theta_angle,enrichment_r):
        ...
        r = enrichment_r.max()
        xy=polar2cartesian(r,theta_angle)

        annot = []
        for kmer,rad in theta_angle.iteritems():
            deg = rad/np.pi*180
            if 90 < deg < 270:
                ha = "right"
                deg -= 180
