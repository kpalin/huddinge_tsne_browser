...
@given(seqs=st.lists(st.sampled_from(list(tsne_obj().sequences[0])),min_size=1,max_size=10,unique=True))
...
@given(seqs=st.lists(st.sampled_from(list(tsne_obj().sequences[0])),min_size=3,max_size=10,unique=True))
def test_layout_write_read(tsne_obj,tmpdir,seqs):
    ...
    assert (loaded_obj.sequences[0].reset_index(drop=True)==tsne_obj.sequences[0].reset_index(drop=True)).all()

    np.testing.assert_allclose(loaded_obj.embedding, tsne_obj.embedding,atol=1e-7,rtol=1e-3)

    np.testing.assert_allclose(loaded_obj.fit_measure_, tsne_obj.fit_measure_,atol=1e-7,rtol=1e-3)
